...
void RegisterTable::operator()()
{
    ...
    const auto& table1 = db->getTable("table1");
    const auto& table2 = db->getTable("table2");

    // Verify integrity of first table.
    compareEQ(table1.getName(), "table1");
    compareEQ(table1.getColumnCount(), static_cast<size_t>(2));
    const auto& col1 = table1.getColumn("col1");
    const auto& col2 = table1.getColumn("col2");
    ...
    const auto& refCol = table2.getColumn("ref");
