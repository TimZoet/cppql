...
void InsertNull::operator()()
{
    ...
    // Insert several rows.
    expectNoThrow([&] { table.insert().compile()(nullptr, 11.0f, sql::toText("abc"), nullptr); });
    expectNoThrow([&] { table.insert().compile()(nullptr, 12.0f, nullptr, 10); });
    expectNoThrow([&] { table.insert().compile()(nullptr, nullptr, sql::toText("def"), 20); });
    expectNoThrow([&] { table.insert<1, 2, 3>().compile()(13.0f, sql::toText("ghi"), 30); });
    expectNoThrow([&] { table.insert<>().compile()(); });
    expectNoThrow([&] { table.insert<3, 2, 1>().compile()(40, sql::toText("jkl"), 14.0f); });
    expectNoThrow([&] { table.insert<1, 3>().compile()(15.0f, 50); });
    expectNoThrow([&] {
        constexpr std::optional<float>       f;
        constexpr std::optional<std::string> s;
        constexpr std::optional<int32_t>     i;
        table.insert().compile()(nullptr, f, i, sql::toStaticText(s));
    });
